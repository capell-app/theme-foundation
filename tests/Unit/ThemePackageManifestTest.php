...
it('registers a Theme Studio definition that matches the manifest', function (): void {
    ...
    $parentTheme = $manifest['extends'] ?? null;

    throw_unless(is_string($themeKey), RuntimeException::class, 'Foundation Theme key must be a string.');
    throw_unless(is_string($packageName), RuntimeException::class, 'Foundation Theme package name must be a string.');
    throw_unless(is_string($parentTheme) || $parentTheme === null, RuntimeException::class, 'Foundation Theme parent must be a string or null.');

    expect($registry->has($themeKey))->toBeTrue();

    $registered = $registry->definition($themeKey);

    expect($registered->key)->toBe($themeKey)
        ->and($registered->key)->toBe(FoundationThemeServiceProvider::THEME_KEY)
        ->and($registered->package)->toBe($packageName)
        ->and($registered->extends)->toBe($parentTheme)
        ->and($registry->has($themeKey))->toBeTrue();
});
...
it('declares standalone theme packages as frontend themes', function (string $packageDirectory, string $composerName, string $themeKey): void {
    ...
    throw_unless(is_string($description), RuntimeException::class, 'Theme package description must be a string.');
    throw_unless(is_array($product), RuntimeException::class, 'Theme package product metadata must be an array.');
    ...
    expect($manifest['name'])->toBe($composerName)
        ...
        ->and($product['group'] ?? null)->toBeIn(['Capell Foundation', 'Capell Themes'])
        ->and(mb_strtolower($description))->not->toContain('generated by capell:make-theme')
        ->and(mb_strtolower($description))->not->toContain('scaffold');
